package com.wenq.strongreferencedemo;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			view = LayoutInflater.from(getContext()).inflate(
					R.layout.photo_layout, null);  //item项布局
		} else {
			view = convertView;
		}
	...
	private void loadBitmaps(int firstVisibleItem, int visibleItemCount) {
		try {
			for (int i = firstVisibleItem; i < firstVisibleItem
				...
				Bitmap bitmap = getBitmapFromMemoryCache(imageUrl);
				if (bitmap == null) {
					BitmapWorkerTask task = new BitmapWorkerTask(); //没有缓存图像，异步下载
					taskCollection.add(task);
					task.execute(imageUrl);
				} else {
					ImageView imageView = (ImageView) mPhotoWall //有缓存图像
							.findViewWithTag(imageUrl); //根据ImageView的Tag属性，寻找ImageView对象
